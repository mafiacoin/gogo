...
import (
	...
	"github.com/dbix-project/go-dubaicoin/common/registrar"
	...
	xe "github.com/dbix-project/go-dubaicoin/xeth"
)
...
func (self *testFrontend) UnlockAccount(acc []byte) bool {
	self.dubaicoin.AccountManager().Unlock(common.BytesToAddress(acc), "password")
	return true
}
...
func testExp(t *testing.T) (dubaicoin *dbix.Dubaicoin, err error) {
	...
	return dbix.New(&node.ServiceContext{EventMux: new(event.TypeMux)}, &dbix.Config{
		AccountManager:          am,
...
func testInit(t *testing.T) (self *testFrontend) {
	// initialise and start minimal dubaicoin stack
	dubaicoin, err := testExp(t)
	if err != nil {
		t.Errorf("error creating dubaicoin: %v", err)
		return
	}
	err = dubaicoin.Start(nil)
	if err != nil {
		t.Errorf("error starting dubaicoin: %v", err)
		return
	}
	...
	self.wait = self.xdbix.UpdateState()
	addr, _ := self.dubaicoin.Etherbase()
	...
	recG := self.xdbix.GetTxReceipt(common.HexToHash(txG))
	if recG == nil {
	...
	recH := self.xdbix.GetTxReceipt(common.HexToHash(txH))
	if recH == nil {
	...
	recU := self.xdbix.GetTxReceipt(common.HexToHash(txU))
	if recU == nil {
...
func TestNatspecE2E(t *testing.T) {
	...
	tf := testInit(t)
	defer tf.dubaicoin.Stop()
	addr, _ := tf.dubaicoin.Etherbase()
	...
	codeb := tf.xdbix.CodeAtBytes(registrar.HashRegAddr)
...
func pendingTransactions(repl *testFrontend, t *testing.T) (txc int64, err error) {
	txs := repl.dubaicoin.TxPool().GetTransactions()
...
func processTxs(repl *testFrontend, t *testing.T, expTxc int) bool {
	...
	err = repl.dubaicoin.StartMining(runtime.NumCPU(), "")
	if err != nil {
		t.Errorf("unexpected error mining: %v", err)
		return false
	}
	defer repl.dubaicoin.StopMining()

	timer := time.NewTimer(100 * time.Second)
	height := new(big.Int).Add(repl.xdbix.CurrentBlock().Number(), big.NewInt(1))
	repl.wait <- height
	select {
	case <-timer.C:
