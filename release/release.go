...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	...
	if err := ctx.Service(&dubaicoin); err != nil {
		return nil, err
	}
	// Construct the release service
	contract, err := NewReleaseOracle(config.Oracle, dbix.NewContractBackend(dubaicoin))
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		case <-timer.C:
			...
			if version.Major > r.config.Major ||
				...
				howtofix := fmt.Sprintf("Please check https://github.com/dbix-project/go-dubaicoin/releases for new releases")
