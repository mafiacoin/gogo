...
// Package xeth is the interface to all Dubaicoin functionality.
package xeth
...
type XEth struct {
	...
	// read-only fields
	backend       *dbix.Dubaicoin
	frontend      Frontend
	agent         *miner.RemoteAgent
	gpo           *dbix.GasPriceOracle
	state         *State
	whisper       *Whisper
	filterManager *filters.FilterSystem
}

func NewTest(dbix *dbix.Dubaicoin, frontend Frontend) *XEth {
	return &XEth{backend: dbix, frontend: frontend}
}

// New creates an XEth that uses the given frontend.
// If a nil Frontend is provided, a default frontend which
// confirms all transactions will be used.
func New(dubaicoin *dbix.Dubaicoin, frontend Frontend) *XEth {
	xeth := &XEth{
		backend:          dubaicoin,
		frontend:         frontend,
		quit:             make(chan struct{}),
		filterManager:    filters.NewFilterSystem(dubaicoin.EventMux()),
		logQueue:         make(map[int]*logQueue),
		blockQueue:       make(map[int]*hashQueue),
		transactionQueue: make(map[int]*hashQueue),
		messages:         make(map[int]*whisperFilter),
		agent:            miner.NewRemoteAgent(),
		gpo:              dbix.NewGasPriceOracle(dubaicoin),
	}
	if dubaicoin.Whisper() != nil {
		xdbix.whisper = NewWhisper(dubaicoin.Whisper())
	}
	dubaicoin.Miner().Register(xdbix.agent)
	if frontend == nil {
		xdbix.frontend = dummyFrontend{}
	}
	state, _ := xdbix.backend.BlockChain().State()
	xdbix.state = NewState(xeth, state)
	go xdbix.start()
	return xeth
}
...
func (self *XEth) WithState(statedb *state.StateDB) *XEth {
	...
	xdbix.state = NewState(xeth, statedb)
	return xeth
}
