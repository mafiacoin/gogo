...
func newWorker(config *core.ChainConfig, coinbase common.Address, dbix core.Backend) *worker {
	worker := &worker{
		config:         config,
		dbix:            dbix,
		mux:            dbix.EventMux(),
		chainDb:        dbix.ChainDb(),
		recv:           make(chan *Result, resultQueueSize),
		gasPrice:       new(big.Int),
		chain:          dbix.BlockChain(),
		proc:           dbix.BlockChain().Validator(),
	...
	for {
		for result := range self.recv {
			...
			if self.fullValidation {
				...
				auxValidator := self.dbix.BlockChain().AuxValidator()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	accounts := self.dbix.AccountManager().Accounts()
...
func (self *worker) commitNewWork() {
	...
	txs := types.NewTransactionsByPriceAndNonce(self.dbix.TxPool().Pending())
	work.commitTransactions(self.mux, txs, self.gasPrice, self.chain)
	self.dbix.TxPool().RemoveBatch(work.lowGasTxs)
	self.dbix.TxPool().RemoveBatch(work.failedTxs)
