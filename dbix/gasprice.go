...
func NewGasPriceOracle(dbix *Dubaicoin) *GasPriceOracle {
	minprice := dbix.GpoMinGasPrice
	if minprice == nil {
		minprice = big.NewInt(gpoDefaultMinGasPrice)
	}
	minbase := new(big.Int).Mul(minprice, big.NewInt(100))
	if dbix.GpobaseCorrectionFactor > 0 {
		minbase = minbase.Div(minbase, big.NewInt(int64(dbix.GpobaseCorrectionFactor)))
	}
...
func (gpo *GasPriceOracle) init() {
	gpo.initOnce.Do(func() {
		gpo.processPastBlocks(gpo.dbix.BlockChain())
		go gpo.listenLoop()
	})
}
...
func (self *GasPriceOracle) listenLoop() {
	events := self.dbix.EventMux().Subscribe(core.ChainEvent{}, core.ChainSplitEvent{})
...
func (self *GasPriceOracle) processBlock(block *types.Block) {
	...
	if lastBase.Cmp(lp) < 0 {
		corr = self.dbix.GpobaseStepUp
	} else {
		corr = -self.dbix.GpobaseStepDown
	}
...
func (self *GasPriceOracle) lowestPrice(block *types.Block) *big.Int {
	...
	receipts := core.GetBlockReceipts(self.dbix.ChainDb(), block.Hash())
	if len(receipts) > 0 {
	...
	if new(big.Int).Mul(gasUsed, big.NewInt(100)).Cmp(new(big.Int).Mul(block.GasLimit(),
		big.NewInt(int64(self.dbix.GpoFullBlockRatio)))) < 0 {
...
func (self *GasPriceOracle) SuggestPrice() *big.Int {
	...
	price.Mul(price, big.NewInt(int64(self.dbix.GpobaseCorrectionFactor)))
	price.Div(price, big.NewInt(100))
	if price.Cmp(self.minPrice) < 0 {
		price.Set(self.minPrice)
	} else if self.dbix.GpoMaxGasPrice != nil && price.Cmp(self.dbix.GpoMaxGasPrice) > 0 {
		price.Set(self.dbix.GpoMaxGasPrice)
	}
	return price
}
