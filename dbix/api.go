...
// PublicEthereumAPI provides an API to access Dubaicoin related information.
...
// NewPublicEthereumAPI creates a new Dubaicoin protocol API.
func NewPublicEthereumAPI(e *Dubaicoin) *PublicEthereumAPI {
...
func NewPublicMinerAPI(e *Dubaicoin) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Dubaicoin) *PrivateMinerAPI {
...
func NewPublicTxPoolAPI(e *Dubaicoin) *PublicTxPoolAPI {
...
func NewPrivateAccountAPI(e *Dubaicoin) *PrivateAccountAPI {
...
func NewPublicTransactionPoolAPI(e *Dubaicoin) *PublicTransactionPoolAPI {
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.dbix.BlockChain().Export(out); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.dbix.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.dbix.BlockChain().InsertChain(blocks); err != nil {
...
// of the Dubaicoin service.
func NewPublicDebugAPI(dbix *Dubaicoin) *PublicDebugAPI {
	return &PublicDebugAPI{dbix: dbix}
}

// DumpBlock retrieves the entire state of the database at a given block.
func (api *PublicDebugAPI) DumpBlock(number uint64) (state.Dump, error) {
	block := api.dbix.BlockChain().GetBlockByNumber(number)
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", number)
	}
	stateDb, err := api.dbix.BlockChain().StateAt(block.Root())
	if err != nil {
...
// of the Dubaicoin service.
func NewPrivateDebugAPI(config *core.ChainConfig, dbix *Dubaicoin) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, dbix: dbix}
}

// ChaindbProperty returns leveldb properties of the chain database.
func (api *PrivateDebugAPI) ChaindbProperty(property string) (string, error) {
	ldb, ok := api.dbix.chainDb.(interface {
		LDB() *leveldb.DB
	})
	if !ok {
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.Config) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.dbix.BlockChain().GetBlock(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(block *types.Block, config *vm.Config) (bool, []vm.StructLog, error) {
	...
	var (
		blockchain = api.dbix.BlockChain()
...
func (api *PrivateDebugAPI) SetHead(number uint64) {
	api.dbix.BlockChain().SetHead(number)
}
...
func (api *PrivateDebugAPI) TraceTransaction(txHash common.Hash, logger *vm.LogConfig) (*ExecutionResult, error) {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(api.dbix.ChainDb(), txHash)
	if tx == nil {
		return nil, fmt.Errorf("transaction %x not found", txHash)
	}
	block := api.dbix.BlockChain().GetBlock(blockHash)
	if block == nil {
		return nil, fmt.Errorf("block %x not found", blockHash)
	}
	// Create the state database to mutate and eventually trace
	parent := api.dbix.BlockChain().GetBlock(block.ParentHash())
	if parent == nil {
		return nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	stateDb, err := api.dbix.BlockChain().StateAt(parent.Root())
	if err != nil {
		return nil, err
	}
	...
	for idx, tx := range block.Transactions() {
		...
		if uint64(idx) < txIndex {
			vmenv := core.NewEnv(stateDb, api.config, api.dbix.BlockChain(), msg, block.Header(), vm.Config{})
		...
		vmenv := core.NewEnv(stateDb, api.config, api.dbix.BlockChain(), msg, block.Header(), vm.Config{Debug: true, Logger: *logger})
