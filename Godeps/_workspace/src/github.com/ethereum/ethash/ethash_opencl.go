...
func PrintDevices() {
	...
	if len(found) == 0 {
		...
		fmt.Printf("Found %v devices. Benchmark first GPU:       gdbix gpubench 0\n", len(found))
		fmt.Printf("Mine using all GPUs:                        gdbix --minegpu %v\n", idsFormat)
	}
}
...
func InitCL(blockNum uint64, c *OpenCLMiner) error {
	platforms, err := cl.GetPlatforms()
	if err != nil {
		return fmt.Errorf("Plaform error: %v\nCheck your OpenCL installation and then run gdbix gpuinfo", err)
	}
	...
	for _, p := range platforms {
		...
		if err != nil {
			return fmt.Errorf("Devices error: %v\nCheck your GPU drivers and then run gdbix gpuinfo", err)
		}
		for _, d := range ds {
	...
	for _, id := range c.deviceIds {
		if id > len(devices)-1 {
			return fmt.Errorf("Device id not found. See available device ids with: gdbix gpuinfo")
		} else {
