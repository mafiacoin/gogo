...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-dubaicoin/build/test/createRandomStateTest\" dubaicoin/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pyethereum
	OUTPUT_PYTHON="$(docker run --rm dubaicoin/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm dubaicoin/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm dubaicoin/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
